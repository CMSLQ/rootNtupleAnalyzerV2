...
def process_input_dir(inputDir, match, filelist, useCERNEOS, eosHost):
    ...
    for fullfilepath in filenamelist:
        ...
        if "powheg" in path and "powheg" not in filename:
            dataset += "_powheg"
        if "amcnloFXFX" in path and "amcnloFXFX" not in filename and "amcatnloFXFX" not in filename:
            dataset += "_amcatnloFXFX"  # intentionally adding 'at' for consistancy with 'amcatnlo' tags above
        if "madgraphMLM" in path and "madgraphMLM" not in filename:
            dataset += "_madgraphMLM"
        if "pythia8" in path and "pythia8" not in filename:
            dataset += "_pythia8"
        # for data, add secondary dataset name
        if "Run2015" in path and "Run2015" not in filename:
            dataset += "__"
            dataset += path[path.find("Run2015"): path.find("/", path.find("Run2015"))]

        if dataset not in list(filelist.keys()):
            filelist[dataset] = []
...
def write_inputlists(filelist, outputDir):
    ...
    for dataset in sorted(filelist.keys()):
...
def combineExtDatasets(filelist):
    ...
    datasetsToRemove = []
    wasCombined = {}
    for index, dataset1 in enumerate(sorted(filelist.keys())):
        ...
        for i in range(index + 1, len(list(filelist.keys()))):
            # if verbose:
            #     print 'consider index:', i, 'out of total entries:', len(filelist.keys())
            dataset2 = sorted(filelist.keys())[i]
